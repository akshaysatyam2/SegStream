...
- **Audio Stream Breaking**: The `AudioContext` was forcing a 48000Hz sample rate which conflicted with some OS-level source device sample rates, causing WebRTC tracks to jitter and drop out after ~5 seconds. Fixed by using the default environment sample rate and adding a `DynamicsCompressorNode` for proper gain staging.
...
6. **Provider Status UI**: Connected the `detect_execution_provider` to the frontend `StatusBar` and `Header` components via context to give real-time visibility into the current ONNX engine (CUDA/OpenVINO/CPU).
7. **Floating Record Controls**: Added a persistent, always-available Floating Action Button (FAB) for recording toggles.

## TODO / Next Steps
- [x] **Inference Optimization**: Migrated to ONNX and added dynamic Execution Providers (CUDA/OpenVINO/CPU).
- [ ] **Documentation**: Write comprehensive `README.md` and module docstrings documenting the new `aiortc` architecture.
- [x] **UI Polish**: Added Floating Record Button and ONNX provider indicators.
